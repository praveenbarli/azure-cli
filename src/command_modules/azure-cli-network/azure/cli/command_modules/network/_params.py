...
def load_arguments(self, _):
    ...
    for item in ag_subresources:
        with self.argument_context('network application-gateway {}'.format(item['name'])) as c:
            c.argument('item_name', options_list=('--name', '-n'), help='The name of the {}.'.format(item['display']), completer=get_ag_subresource_completion_list(self.cli_ctx, item['ref']), id_part='child_name')
    ...
    with self.argument_context('network application-gateway url-path-map rule') as c:
        c.argument('item_name', options_list=('--name', '-n'), help='The name of the url-path-map rule.', completer=get_ag_url_map_rule_completion_list(self.cli_ctx), id_part='grandchild_name')
        c.argument('url_path_map_name', options_list=('--path-map-name',), help='The name of the URL path map.', completer=get_ag_subresource_completion_list(self.cli_ctx, 'url_path_maps'), id_part='child_name')
    ...
    with self.argument_context('network express-route auth') as c:
        ...
        c.argument('authorization_name', name_arg_type, id_part='child_name', help='Authorization name')
    ...
    with self.argument_context('network express-route peering') as c:
        c.argument('circuit_name', circuit_name_type)
        c.argument('peering_name', name_arg_type, id_part='child_name')
        c.argument('peering_type', validator=validate_peering_type, arg_type=get_enum_type(ExpressRouteCircuitPeeringType))
        ...
        c.argument('primary_peer_address_prefix', options_list=['--primary-peer-subnet'])
        c.argument('secondary_peer_address_prefix', options_list=['--secondary-peer-subnet'])
        c.argument('advertised_public_prefixes', arg_group='Microsoft Peering', nargs='+')
        c.argument('customer_asn', arg_group='Microsoft Peering')
        c.argument('routing_registry_name', arg_group='Microsoft Peering', arg_type=get_enum_type(routing_registry_values))
    ...
    for item in lb_subresources:
        with self.argument_context('network lb {}'.format(item['name'])) as c:
            c.argument('item_name', options_list=('--name', '-n'), help='The name of the {}'.format(item['display']), completer=get_lb_subresource_completion_list(self.cli_ctx, item['ref']), id_part='child_name')
    ...
    with self.argument_context('network nic ip-config') as c:
        ...
        c.argument('ip_config_name', options_list=('--name', '-n'), metavar='IP_CONFIG_NAME', help='The name of the IP configuration.', id_part='child_name')
        ...
        c.argument('item_name', options_list=('--name', '-n'), metavar='IP_CONFIG_NAME', help='The name of the IP configuration.', id_part='child_name')
    ...
    for item in ['address-pool', 'inbound-nat-rule']:
        with self.argument_context('network nic ip-config {}'.format(item)) as c:
            c.argument('ip_config_name', options_list=('--ip-config-name', '-n'), metavar='IP_CONFIG_NAME', help='The name of the IP configuration.', id_part='child_name')
    ...
    with self.argument_context('network nsg rule') as c:
        c.argument('security_rule_name', name_arg_type, id_part='child_name', help='Name of the network security group rule')
    ...
    with self.argument_context('network route-filter rule') as c:
        ...
        c.argument('rule_name', name_arg_type, help='Name of the route filter rule.', id_part='child_name')
    ...
    with self.argument_context('network route-table route') as c:
        c.argument('route_name', name_arg_type, id_part='child_name', help='Route name')
    ...
    with self.argument_context('network traffic-manager profile') as c:
        ...
        c.argument('monitor_protocol', help='Monitor protocol.', arg_type=get_enum_type(MonitorProtocol))
    ...
    with self.argument_context('network traffic-manager endpoint') as c:
        c.argument('endpoint_name', name_arg_type, id_part='child_name', help='Endpoint name.', completer=get_tm_endpoint_completion_list())
        c.argument('endpoint_type', options_list=['--type', '-t'], help='Endpoint type.', id_part='child_name', arg_type=get_enum_type(endpoint_types))
    ...
    with self.argument_context('network vnet peering') as c:
        ...
        c.argument('virtual_network_peering_name', options_list=('--name', '-n'), help='The name of the VNet peering.', id_part='child_name')
    ...
    with self.argument_context('network vnet subnet') as c:
        c.argument('subnet_name', arg_type=subnet_name_type, options_list=('--name', '-n'), id_part='child_name')
