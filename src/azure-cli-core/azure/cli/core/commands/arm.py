...
def _cli_generic_update_command(context, name, getter_op, setter_op, setter_arg_name='parameters',
    ...
    def function_arguments_loader():
        ...
        doc_string_source = kwargs.get('doc_string_source', None)
        if doc_string_source:
            custom_op.__doc__ = doc_string_source.__doc__
    ...
    def handler(args):  # pylint: disable=too-many-branches,too-many-statements
        ...
        factory = cmd.command_kwargs.get('client_factory', None)
        client = None
        if factory:
            try:
        ...
        client_arg_name = 'client' if cmd.command_kwargs['operations_tmpl'].startswith('azure.cli') else 'self'
