...
def get_enum_type(data, default=None):
    ...
    default = next((x for x in choices if x.lower() == default.lower()), None) if default else None

    return CLIArgumentType(choices=CaseInsensitiveList(choices), type=_type, default=default)
